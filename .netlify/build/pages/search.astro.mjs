...
</h3> <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6" role="grid" aria-label="Matching MOD APKs" data-astro-cid-ipsxrsrh> ${searchResults.apps.map((app, index) => renderTemplate`<article role="gridcell" class="app-card group relative bg-white dark:bg-gray-800 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-700 focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2 dark:focus-within:ring-offset-gray-900"${addAttribute(app.name, "data-app-name")}${addAttribute(app.category, "data-app-category")} data-astro-cid-ipsxrsrh> <a${addAttribute(`/${app.slug}`, "href")} class="block w-full h-full focus:outline-none"${addAttribute(`Download ${app.name} v${app.version} MOD APK - ${app.category} app with premium features unlocked`, "aria-label")} data-astro-cid-ipsxrsrh> <div class="relative h-32 sm:h-40 bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center overflow-hidden group-hover:scale-105 transition-transform duration-300" data-astro-cid-ipsxrsrh> <!-- Status Badges --> <div class="absolute top-2 left-2 flex gap-1.5 z-10" data-astro-cid-ipsxrsrh> <span class="bg-blue-500 text-white text-xs font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 shadow-sm" data-astro-cid-ipsxrsrh> <svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true" data-astro-cid-ipsxrsrh> <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" data-astro-cid-ipsxrsrh></path> </svg>
MATCH
