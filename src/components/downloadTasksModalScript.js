...
function renderTasks() {
  ...
  if (tasks.length === 0) {
    ...
    // Update tasks list
    tasksList.innerHTML = `
      <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400 py-3 sm:py-4">
        No tasks required. Click the button below to download.
      </p>
    `;
  ...
  const tasksHtml = tasks.filter(Boolean).map((task, index) => {
    ...
    return `
      <button
        class="task-button w-full py-3 sm:py-4 px-4 sm:px-6 rounded-xl flex items-center justify-between ${taskTypeClass} ${isCompleted ? 'opacity-75' : ''}"
      ...
      >
        <div class="flex items-center">
          ...
          <span class="ml-2 sm:ml-3 font-medium text-white text-sm sm:text-base truncate max-w-[150px] sm:max-w-[200px]">${task.name}</span>
        </div>
        ${isCompleted ? '<svg class="w-5 h-5 sm:w-6 sm:h-6 text-white flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>' : ''}
      </button>
    `;
  }).join('');
...
function handleTaskClick(e) {
  try {
    const button = e.currentTarget || e.target.closest('.task-button');
    if (!button) return;
    ...
    if (taskId && !completedTasks.includes(taskId)) {
      ...
      // Add completed styles - ensure we're using responsive classes
      button.classList.add('opacity-75');
      button.innerHTML = button.innerHTML.replace(/<\/div>(?!.*<\/div>)/s, '</div><svg class="w-5 h-5 sm:w-6 sm:h-6 text-white flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.addEventListener('load', function() {
    ...
    setTimeout(initializeModal, 150);
  });
  ...
  window.addEventListener('resize', function() {
    ...
    this.resizeTimeout = setTimeout(function() {
      // Check if modal is open before reinitializing
...
function getTaskTypeIcon(taskType) {
  switch (taskType?.toLowerCase()) {
    case 'telegram':
      return `<svg class="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
    ...
    default:
      return `<svg class="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
